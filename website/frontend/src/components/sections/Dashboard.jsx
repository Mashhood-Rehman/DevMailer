...
const Dashboard = ({ user }) => {
  ...
  const handleCheckout = async () => {
    try {
      ...
      if (response.status === 401) {
        window.location.href = `${import.meta.env.VITE_API_BACKEND_URL}/auth/google?redirect=${encodeURIComponent(window.location.origin)}`;
        return;
      }
  ...
  const handleManageSubscription = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BACKEND_URL}/payments/customer-portal`, {
        credentials: 'include'
      });
      const data = await response.json();
      if (data.url) {
                ...
                <div className="flex flex-col sm:flex-row gap-4 pt-4">
                  {user.tier !== 'PREMIUM' ? (
                    ...
                    <button
                      ...
                      className="grow bg-slate-700/50 text-slate-300 py-3.5 rounded-2xl font-bold text-center border border-slate-700 hover:bg-slate-700 hover:text-white transition-all active:scale-95"
                    >
                      Manage Subscription 💎
                    </button>
                  )}
